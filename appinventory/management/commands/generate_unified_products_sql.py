...
class Command(BaseCommand):
    help = 'Generate MySQL INSERT statements for unified products'

    def add_arguments(self, parser):
        parser.add_argument(
            '--output-file',
            type=str,
            default='db/insert_unified_products.sql',
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f'[INFO] Generating MySQL INSERT statements...'))
        ...
        sql_statements = []
        sql_statements.append('-- MySQL INSERT statements for unified products')
        ...
        sql_statements.append('-- Deactivate original products (commented out for safety)')
        sql_statements.append('-- UPDATE appinventory_product SET is_active = 0 WHERE id IN (')
        
        # Obtener IDs de productos originales desactivados
        original_products = Product.objects.filter(is_active=False).order_by('id')
        original_ids = [str(p.id) for p in original_products]
        
        if original_ids:
            # Dividir en chunks de 50 para evitar líneas muy largas
            chunk_size = 50
            for i in range(0, len(original_ids), chunk_size):
                chunk = original_ids[i:i + chunk_size]
                sql_statements.append('-- ' + ', '.join(chunk) + (',' if i + chunk_size < len(original_ids) else ''))
        ...
        self.stdout.write(self.style.SUCCESS(f'[SUCCESS] SQL file created successfully!'))
